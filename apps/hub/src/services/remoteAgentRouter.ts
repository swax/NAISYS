...
export function createRemoteAgentRouter(
  ...
  function forwardRequest<TRes extends RemoteResponse>(
    ...
    if (data.targetHostId === sourceHostId) {
      logService.log(
        `[RemoteAgent] Target host is source host, should be handled locally`,
      );
      ack?.({
        ...errorResponse,
        error: "Target host is source host - handle locally",
      });
      return;
    }
    ...
    if (!hubServer.getConnectionByHostId(data.targetHostId)) {
      logService.log(
        `[RemoteAgent] Target host ${data.targetHostId} not connected`,
      );
      ack?.({
        ...errorResponse,
        error: `Target host ${data.targetHostId} not connected`,
      });
      return;
    }
    ...
    const sent = hubServer.sendMessage<TRes>(
      ...
      (rawResponse: unknown) => {
        ...
        if (!respResult.success) {
          ...
          ack?.({
            ...errorResponse,
            error: "Invalid response from target host",
          });
          return;
        }
    ...
    if (!sent) {
      logService.log(
        `[RemoteAgent] Failed to send agent ${operation} request to ${data.targetHostId}`,
      );
      ack?.({
        ...errorResponse,
        error: "Failed to send request to target host",
      });
    }
  }
