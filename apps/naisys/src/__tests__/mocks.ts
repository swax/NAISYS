...
export function createMockShellCommand() {
  const shellCommand: ShellCommand = {
    handleCommand: vi.fn(() =>
      Promise.resolve({ response: "", exitApp: false }),
    ),
...
export function createMockContextManager() {
  ...
  const contextManager: ContextManager = {
    ...
    appendImage,
    appendAudio,
    appendDesktopRequest: vi.fn(),
    appendDesktopResult: vi.fn(),
    appendDesktopError: vi.fn(),
...
export function createMockGlobalConfig(): GlobalConfig {
  return {
    ...
    patchVariableValues: vi.fn(() => Promise.resolve()),
    globalConfig: () => ({
      hostname: "test",
      shellCommand: {
        outputTokenMax: 7500,
